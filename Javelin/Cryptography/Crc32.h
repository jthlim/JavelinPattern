...
//============================================================================
	...
	namespace Private
	{
		...
		JINLINE constexpr unsigned Crc32Iteration(unsigned hash, unsigned char p) 	{ return CRC32_TABLE[(hash ^ p) & 0xff] ^ (hash >> 8); }
		template<int n> constexpr unsigned Crc32(const unsigned char p[])			{ return Crc32Iteration(Crc32<n-1>(p), p[n]); }
		template<> constexpr unsigned Crc32<-1>(const unsigned char[])				{ return 0xffffffff; }
	}
