...
//============================================================================
	...
	namespace Private
	{
		...
		JINLINE constexpr uint64_t Crc64Iteration(uint64_t hash, char p) 	{ return CRC64_TABLE[(hash >> 56) ^ (p&0xff)] ^ (hash << 8); }
		template<int n> constexpr uint64_t Crc64(const char p[])			{ return Crc64Iteration(Crc64<n-1>(p), p[n]); }
		template<> constexpr uint64_t Crc64<-1>(const char[])				{ return 0xffffffffffffffffull; }
	}
