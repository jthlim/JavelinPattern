...
#include "Javelin/Tools/jasm/riscv/Tokenizer.h"
...
void Assembler::ParseSegment(Tokenizer &tokenizer, const std::vector<std::string> &filenameList)
{
	for(;;)
	{
		...
		catch(AssemblerException &ex)
		{
            ...
            if (ex.empty())
            {
                Log::Error("Empty exception thrown");
            }
            else
            {
                for(const auto &it : ex) Log::Error("%s", it.GetLogString(filenameList).c_str());
...
{
	Token token = tokenizer.PeekToken();
	if(token.sValue == "signed12")
	{
		tokenizer.NextToken();
		token = tokenizer.GetToken();
		if(token.type != Token::Type::Expression)
		{
			throw AssemblerException("Unexpected token: %s (Expected expression for signed12 condition)", token.GetDescription().c_str());
		}
...
void Assembler::ParseInstruction(ListAction& listAction, Tokenizer &tokenizer, const Token &instructionToken)
{
    ...
    // Resolve RepeatOp cases
    for(int i = currentOperandIndex-1; i >= 2; --i)
    {
        if(operands[i]->type == Operand::Type::Register)
        {
            MatchBitfield extraMatch = 0;
            if(operands[i] == operands[0]) extraMatch |= MatchRepeatOp0;
            if(operands[i] == operands[1]) extraMatch |= MatchRepeatOp1;

            if(extraMatch != 0)
            {
                // Need to tag the register as MatchRepeatOpN, so take a copy first.
...
bool Assembler::IsEquivalentByteCode(const Assembler& other) const
{
    if(!rootListAction.HasData()) return !other.rootListAction.HasData();
    if(!other.rootListAction.HasData()) return false;

    if (isDataSegment != other.isDataSegment) return false;
    
    ActionWriteContext context(isDataSegment, appendAssemblerReferenceSize, CommandLine::GetInstance().GetAssemblerVariableName());
    ActionWriteContext otherContext(other.isDataSegment, appendAssemblerReferenceSize, CommandLine::GetInstance().GetAssemblerVariableName());

    int expressionOffset = AddExpressionInfoToContext(context);
    int otherExpressionOffset = other.AddExpressionInfoToContext(otherContext);
    if (expressionOffset != otherExpressionOffset) return false;

    if (rootListAction.GetMaximumLength() != other.rootListAction.GetMaximumLength()) return false;

    std::vector<uint8_t> bytes;
    rootListAction.WriteByteCode(bytes, context);

    std::vector<uint8_t> otherBytes;
    rootListAction.WriteByteCode(otherBytes, context);
    
    if(context.numberOfLabels != otherContext.numberOfLabels) return false;
    if(context.GetNumberOfForwardLabelReferences() != otherContext.GetNumberOfForwardLabelReferences()) return false;
    if(bytes != otherBytes) return false;

    return true;
}

int Assembler::AddExpressionInfoToContext(ActionWriteContext &context) const
{
    int expressionOffset = 0;
...
void Assembler::Write(FILE *f, int startingLine, int *expectedFileIndex, const std::vector<std::string> &filenameList) const
{
	...
	std::vector<uint8_t> bytes;
	rootListAction.WriteByteCode(bytes, context);
	bytes.push_back((int) riscvAssembler::ActionType::Return);
	...
	if(expressionOffset != 0)
	{
		...
		// These fields are from riscv Assembler::AppendAssemblyReference.
...
Operand* Assembler::ParseOperandPrimary(Tokenizer &tokenizer, AutoDeleteVector &temporaryOperands)
{
	...
	uint64_t labelMatch = MatchRelAll;
Next:
	Token token = tokenizer.PeekToken();
	int lineNumber = tokenizer.GetCurrentLineNumber();
	int fileIndex = tokenizer.GetCurrentFileIndex();
    ...
    case Token::Type::RoundingMode:
        tokenizer.NextToken();
        return (Operand *)token.immediateOperand;
	case Token::Type::IntegerValue:
		{
			tokenizer.NextToken();
		ParseNumericLabel:
			...
			ProcessIntegerValue:
                ...
                immediateOperand->UpdateMatchBitfield();
	...
	case Token::Type::RealValue:
		{
			...
			temporaryOperands.push_back(immediateOperand);
			immediateOperand->matchBitfield = MatchFloatImm;
			return immediateOperand;
		}
	case Token::Type::Expression:
		{
		...
		ParseExpressionLabel:
			...
			immediateOperand->matchBitfield = MatchImmAll | MatchRelAll;
			return immediateOperand;
		}
	case Token::Type::Star:
		{
	...
	case Token::Type::OpenParenthesis:
		{
            ...
            if (result->matchBitfield & MatchRegX)
            {
                DeleteWrapper<RegisterOperand> *registerOperand = new DeleteWrapper<RegisterOperand>(*static_cast<RegisterOperand*>(result));
                result = registerOperand;
                registerOperand->matchBitfield ^= MatchRegX | MatchParenthesizedRegX;
            }
			return result;
		}
